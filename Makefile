...
OS := $(shell uname -s)
# Accept both app-directory overrides for existing install commands.
MAC_APPDIR ?= $(HOME)/Applications
APPLICATIONS ?= $(MAC_APPDIR)
ifeq ($(OS),Darwin)
PLATFORM = macos
else
PLATFORM = linux
...
.PHONY: help build run install uninstall install-linux uninstall-linux \
	install-macos uninstall-macos app bundle lint test ci fmt clean
...
help:
	...
	@echo "  make bundle      alias for make app"
...
install-macos: app
	...
	ditto "$(APP)" "$(APPLICATIONS)/disktree.app"
...
app install-macos uninstall-macos:
	@echo "App bundles must be built and installed on macOS"; exit 1
endif
